...
export async function addRentalMarketFee(formData: FormData) {
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    return { error: "Enter a valid fee amount greater than zero" };
  }
  const date = parseDateInput(dateRaw);
  if (!date) return { error: "Choose a valid date" };
  if (!vendor) return { error: "Enter who you paid (market, landlord, stall holder…)" };
...
export async function deleteRentalMarketFee(transactionId: string) {
  const businessId = await getBusinessId();
  const category = await ensureRentalMarketFeesCategory(businessId);
  ...
  if (!tx) return { error: "Fee entry not found" };
